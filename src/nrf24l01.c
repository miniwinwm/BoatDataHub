...
void nrf24l01_spi_send_read(unsigned char * data, unsigned int len, bool copydata)
{
	...
	{
		if(copydata != false)
			data[count] = spi2_send_read_byte(data[count]);
		else
		{
			tempbyte = data[count];
			spi2_send_read_byte(tempbyte);
		}
	}
}
...
{
	bool irq=!GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_8);
	return irq;
}
...
{
	nrf24l01_set_ce();
	// taken 1 10us delay out from here as doesn't seem to be needed
	nrf24l01_clear_ce();
}
